...
import { TFavoriteButtonProps } from '../types/types'
import './FavoriteButton.css'
import { useAppDispath } from '../services/hooks'
import { fetchLikedProduct } from '../services/product/productSlice'

const FavoriteButton = ({ productId, likes, userId }: TFavoriteButtonProps) => {
	...
	useEffect(() => {
		if (userId && likes)
			setIsFavorite(likes.some((user) => user._id === userId))
	}, [userId, likes])

	const handleClick = (event: BaseSyntheticEvent) => {
		event.preventDefault()
		if (productId && likes) {
			dispatch(fetchLikedProduct({ productId, like: isFavorite })).then(() => {
	...
	return (
		<button
			className={`card__favorite ${isFavorite ? 'card__favorite_is-active' : ''}`}
			onClick={handleClick}
		>
			<svg
				width='24'
